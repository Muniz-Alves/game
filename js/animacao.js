...
// Obtém a referência para o botão "start-button"
const startButton = document.getElementById('start-button');

// Adiciona um event listener para o botão "start-button"
startButton.addEventListener('click', function () {
  // Inicia o intervalo de tempo para chamar a função animateBall a cada 10 milissegundos
  animateBall();
  startButton.parentNode.removeChild(startButton);
  let tempoTotal = 300;

  function atualizarTempo() {
    // Decrementa um segundo do tempo total
    tempoTotal--;
  
    // Calcula os minutos e segundos a partir do tempo total
    ...
    // Atualiza o conteúdo do elemento div com o tempo atual
    document.getElementById('cronometro').innerHTML = tempoFormatado;
  
    // Cancela o intervalo quando o tempo chegar a zero
    if (tempoTotal === -1) {
  ...
  // Inicia o intervalo para atualizar o tempo a cada segundo
...
const botaoCimaElement = document.querySelector('#botao-cima');
const botaoBaixoElement = document.querySelector('#botao-baixo');


botaoCimaElement.addEventListener('click', moveGoleiro);
botaoBaixoElement.addEventListener('click', moveGoleiro);

function moveGoleiro(event) {
  // Obtém a direção do botão pressionado
  const direcao = event.target.id === 'botao-cima' ? -1 : 1;

  // Calcula a nova posição do goleiro baseada na direção do botão
  const novaPosicao = goleiroElement.offsetTop + (direcao * 20);
